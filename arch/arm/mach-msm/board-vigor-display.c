...
void __init vigor_allocate_fb_region(void)
{
	unsigned long size;
...
struct mdp_table_entry vigor_color_v11[] = {
	{0x93400, 0x0222, 0x0},
	{0x93404, 0xFFE4, 0x0},
	{0x93408, 0xFFFD, 0x0},
	{0x9340C, 0xFFF1, 0x0},
...
{
  mdp_color_enhancement(vigor_color_v11, ARRAY_SIZE(vigor_color_v11));
  mdp_color_enhancement(mdp_sharp_barrier_off, ARRAY_SIZE(mdp_sharp_barrier_off));

  return 0;
}

int vigor_mdp_gamma_cool(void)
{
...
static struct msm_panel_common_pdata mdp_pdata = {
	...
	.mdp_gamma_cool = vigor_mdp_gamma_cool,
};
...
static struct dsi_cmd_desc vig_sharp_cmd_on_cmds[] = {
...
static int vigor_lcd_on(struct platform_device *pdev)
{
  ...
  if (panel_type == PANEL_ID_VIG_SHARP_NT) {
			printk(KERN_INFO "vigor_lcd_on PANEL_ID_VIG_SHARP_HX\n");
			cmdreq.cmds = VIG_SHARP_cmd_on_cmds;
			cmdreq.cmds_cnt = ARRAY_SIZE(vig_sharp_cmd_on_cmds);
...
static int vigor_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static void vigor_set_backlight(struct msm_fb_data_type *mfd)
{
	if (panel_type == PANEL_ID_VIG_SHARP_HX) {
		led_pwm1[1] = vigor_shrink_pwm((unsigned char)(mfd->bl_level));
	}
	else {
...
static int __devinit vigor_lcd_probe(struct platform_device *pdev)
{
...
static struct msm_fb_panel_data vigor_panel_data = {
	.on	       = vigor_lcd_on,
	.off	       = vigor_lcd_off,
	.set_backlight = vigor_set_backlight,
};

static struct msm_panel_info pinfo;
static int ch_used[3] = {0, 0, 0};

static int mipi_vigor_device_register(const char* dev_name, struct msm_panel_info *pinfo,
...
static int __init mipi_cmd_novatek_blue_qhd_pt_init(void)
{
	...
	ret = mipi_vigor_device_register("mipi_novatek", &pinfo, MIPI_DSI_PRIM,
...
device_initcall_sync(vigor_panel_init);

/* 3D Panel */
static void vigor_3Dpanel_on(bool bLandscape)
{
...
//	led_brightness_value_set("lcd-backlight", 255);
	...
	if (system_rev >= 1) {
		...
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(VIGOR_3DLCM_PD), &pwm_gpio_config);
		if (rc < 0)
			pr_err("%s pmic gpio config gpio %d failed\n", __func__, PM8058_GPIO_PM_TO_SYS(VIGOR_3DLCM_PD));
	}

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(VIGOR_3DCLK), &clk_gpio_config_on);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, VIGOR_3DCLK);
	...
	if (bLandscape) {
		gpio_set_value(VIGOR_CTL_3D_1, 1);
		gpio_set_value(VIGOR_CTL_3D_2, 1);
		gpio_set_value(VIGOR_CTL_3D_3, 1);
		gpio_set_value(VIGOR_CTL_3D_4, 0);
	} else {
		gpio_set_value(VIGOR_CTL_3D_1, 1);
		gpio_set_value(VIGOR_CTL_3D_2, 1);
		gpio_set_value(VIGOR_CTL_3D_3, 0);
		gpio_set_value(VIGOR_CTL_3D_4, 1);
	}
}

static void vigor_3Dpanel_off(void)
{
	int rc;
	if (system_rev >= 1) {
		...
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(VIGOR_3DLCM_PD), &pwm_gpio_config);
		if (rc < 0)
			pr_err("%s pmic gpio config gpio %d failed\n", __func__, VIGOR_3DLCM_PD);
	}
	pwm_disable(pwm_3d);

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(VIGOR_3DCLK), &clk_gpio_config_off);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, VIGOR_3DCLK);
	gpio_set_value(VIGOR_CTL_3D_1, 0);
	gpio_set_value(VIGOR_CTL_3D_2, 0);
	gpio_set_value(VIGOR_CTL_3D_3, 0);
	gpio_set_value(VIGOR_CTL_3D_4, 0);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case BARRIER_LANDSCAPE:
		vigor_3Dpanel_on(true);
		break;
	case BARRIER_PORTRAIT:
		vigor_3Dpanel_on(false);
		break;
	default:
		break;
	}
...
static int vigor_3Dpanel_probe(struct platform_device *pdev)
{
	int err = 0;
	printk(KERN_INFO "%s\n", __func__);
...
static int vigor_3Dpanel_remove(struct platform_device *pdev)
{
	printk(KERN_INFO "%s\n", __func__);
	if (pwm_3d) {
		pwm_free(pwm_3d);
...
struct platform_driver vigor_3Dpanel_driver = {
	.probe	= vigor_3Dpanel_probe,
	.remove	= vigor_3Dpanel_remove,
	.driver	= {
		.name = "panel_3d",
	},
};

static int __init vigor_3Dpanel_init(void)
{
	pr_info("%s(%d)\n", __func__, __LINE__);
	return platform_driver_register(&vigor_3Dpanel_driver);
}

static void __exit vigor_3Dpanel_exit(void)
{
	platform_driver_unregister(&vigor_3Dpanel_driver);
}

module_init(vigor_3Dpanel_init);
module_exit(vigor_3Dpanel_exit);
